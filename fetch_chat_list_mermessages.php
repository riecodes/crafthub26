...
$query = "  SELECT
            ...
            WHERE
                ...
                AND crafthub_users.role = 'customer'
            GROUP BY
...
if ($stmt) {
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($chat = mysqli_fetch_assoc($result)) {
            ...
            echo '
            <li class="chat-item ' . $unread_class . '" onclick="selectChat(this, \'' . htmlspecialchars($chat['chat_with_username']) . '\',  \'' . htmlspecialchars($chat['chat_with_id']) . '\', \'' . htmlspecialchars($last_seen_display) . '\')">
